...
Si au tour précédent le Character Rois a été associé à un Behaviour, alors il choisira son rôle en premier (cf. updateListOfBehaviour())
Une fois que chaque Behaviour est associé à un Character, le RoundManager va demander à chaque rôle les uns après les autres de jouer leurs tours (cf. askEachCharacterToPlay())
Une fois que chaque Character à joué, le RoundManager regarde si un des joueurs a complété sa ville. Si personne n'a terminé sa ville, un nouveau tour commence. Dans le cas où quelqu'un a terminé sa ville, le RoundManagel arrête la partie et renvoie une liste de Behaviour qui ont terminé leurs ville, classé du premier au dernier à l'avoir complété. 
